...
def uncover_spy(n, trust):
    ...
    trusted_dict = {}
    trusters_set = set()
    # places all pairs in pair_dict
    for index, pair in enumerate(trust):
        pair_dict[index] = pair

    # iterate over pair_dict with key as index
    for key in pair_dict:
        # creates a set of trusted citizens (2nd index in each pair)
        trusted = pair_dict[key][1]
        # fills trusters_set with the trusting people (1st index in each pair)
        trusters_set.add(pair_dict[key][0])

        # increments trust count for each citizen that's trusted
        if trusted not in trusted_dict:
            trusted_dict[trusted] = 1
        else:
            trusted_dict[trusted] += 1
    # iterates over dict of trusted
    for key in trusted_dict:
        # if the key is not found amongst the trusters
        if key not in trusters_set:
            if trusted_dict[key] == n-1:
                # the key is returned, the spy is found
                return key
            else:
                return -1
        else:
